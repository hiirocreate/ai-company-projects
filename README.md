...
- 社員E(開発担当)は、MVPの中核機能を実際に確認できる**単一HTMLファイルのプロトタイプコード**を
  生成します。検証用の簡易プロトタイプであり、本番運用コードではありません。「📋 コピー」で
  そのままコピーして自分のエディタ等に貼り付けられます。
- 社員F(営業担当)は、**そのままコピーして使えるSNS投稿文案(2パターン)と個別アプローチ文面**を
  生成します。実際のSNS投稿・DM送信は行いません(仕様書20章の通り、送信は必ず人間が行ってください)。
- AIの下書きには、前工程の内容に不足や問題がないかをチェックする**確認コメント**(仕様書14章の
  相互監視に対応)もあわせて表示されます。これは記録には保存されず、その場の判断材料としてのみ表示されます。
- オフィス画面では、今まさに作業中の社員のキャラクターが光る演出になり、誰から誰へ仕事が引き継がれて
  いるかが「🔗 ○○と連携中」のバッジと、MEETING ROOM内の矢印表示で分かるようになっています。
  MEETING ROOMは画面上部に固定表示され、社員カードは横並びのコンパクトな表示です。
- 画面上部の「🏢 オフィス / 📋 プロジェクト / 📜 ログ」タブで表示を切り替えられます(スマホでの
  スクロール量を減らすため)。
